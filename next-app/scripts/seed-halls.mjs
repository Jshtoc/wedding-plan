#!/usr/bin/env node
// Seed the `halls` table in Supabase with the 6 initial wedding halls.
// Usage: npm run seed-halls
// Requires: schema.sql has been run in Supabase first.
//
...
// Minimal .env parser — skip comments and blank lines
const env = {};
...
if (!url || !key) {
  console.error(
    "[seed] NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY missing in .env.local"
  );
  process.exit(1);
}
...
// ── Hall seed data (camelCase → will be converted to snake_case below) ──
const halls = [
  {
    name: "제이오스티엘",
    ...
    bestLabel: "★ 가격 최우선 추천",
    image:
      "https://dnna01d8m6k3w.cloudfront.net/partnerProfile/2207/202410/20241022/e4d8fa4e-0507-4293-bbfe-7a02cc64de96.jpg",
  ...
  },
  {
    ...
    parking: 500,
    image:
      "https://dnna01d8m6k3w.cloudfront.net/partnerProfile/2163/202410/20241010/945004a7-0953-49a1-9a5e-9edeebcfb56c.png",
  ...
  },
  {
    ...
    parking: 2000,
    image:
      "https://cdn.prod.website-files.com/66a1eeaa00f1c86c3dbae974/69c25dc732ace8f1e25e17be_%EC%9B%A8%EB%94%A9%EC%8B%9C%ED%8B%B0%20%EC%8A%A4%ED%83%80%ED%8B%B0%EC%8A%A4%20%ED%99%80%EC%9D%B4%EB%AF%B8%EC%A7%8003.jpg",
  ...
  },
  {
    ...
    parking: 500,
    image:
      "https://cdn.prod.website-files.com/66a1eeaa00f1c86c3dbae974/66f60cdffac1fc0d13ffda17_1339047902_img_6174_0_1726195665.avif",
  ...
  },
  {
    ...
    parking: 600,
    image:
      "https://cdn.prod.website-files.com/66a1eeaa00f1c86c3dbae974/68ec9f47955088aa287a2a25_b7a70fdd06035.avif",
  ...
  },
  {
    ...
    parking: 300,
    image:
      "https://dnna01d8m6k3w.cloudfront.net/partnerProfile/3506/202410/20241011/750423a6-9d0e-4cda-91a8-a30fd9e97bdd.jpg",
    imageAlt: "아만티호텔 웨딩홀",
    imageFallback: "🕊️",
    badges: [
...
// Convert camelCase → snake_case for DB row shape
function toRow(hall) {
  return {
    name: hall.name,
    sub: hall.sub,
...
console.log(`[seed] Inserting ${halls.length} halls into ${url}...\n`);

let ok = 0;
let fail = 0;
for (const hall of halls) {
  const { error } = await supabase.from("halls").insert(toRow(hall));
  if (error) {
...
console.log(`\n[seed] Done. ${ok} inserted, ${fail} failed.`);
