...
| `USER1_ID` | 서버 전용 | 첫 번째 고정 계정 ID (예: 남편 `wed1`) |
| `USER1_PASSWORD_HASH` | 서버 전용 | bcrypt 해시된 `USER1` 비밀번호 (`npm run hash-password`로 생성) |
| `USER1_ROLE` | 서버 전용 | `USER1` 역할 (기본 `husband`) |
| `USER2_ID` | 서버 전용 | 두 번째 고정 계정 ID (예: 아내 `wed2`) |
| `USER2_PASSWORD_HASH` | 서버 전용 | bcrypt 해시된 `USER2` 비밀번호 |
| `USER2_ROLE` | 서버 전용 | `USER2` 역할 (기본 `wife`) |

이 프로젝트는 정확히 두 개의 고정 계정만 지원합니다. 추가 계정이 필요해지면 `src/lib/authUsers.ts`의 `loadUsers()`에 `USER3_*`, `USER4_*` 블록을 복제해 넣으면 됩니다.
...
### ⚠️ `.env.local`에서 `$` escape 필수

Next.js (`@next/env` + `dotenv-expand`)는 `.env` 파일의 값에 있는 `$`를 환경 변수 참조로 해석한다. 따라서 bcrypt 해시(`$2b$10$...`)나 `$` 문자가 포함된 시크릿은 반드시 **백슬래시로 escape** 해야 한다.

```bash
# ❌ 잘못된 방식 — Next.js가 $2b, $10, $630x...를 변수로 해석해서 잘라먹음
USER1_PASSWORD_HASH=$2b$10$630xnPMSEAL2wm4X8w9zI.SJ1QUGLvCZ2/9IlrqJtaurSsl7lIBh.

# ❌ 단일/이중 인용부호는 무효 — 여전히 expansion 발생
USER1_PASSWORD_HASH='$2b$10$630xnPMSEAL...'
USER1_PASSWORD_HASH="$2b$10$630xnPMSEAL..."
...
**단, Vercel 대시보드의 Environment Variables UI에 붙여넣을 때는 escape 불필요.** Vercel은 자체 파서를 사용하며 `$`를 literal로 저장한다. Vercel에 붙여넣는 값은 원본 해시(`$2b$10$...`) 그대로.

`scripts/hash-password.mjs`가 두 형태(원본 + escape된 것)를 모두 출력하므로 목적지에 맞는 쪽을 복사해 쓰면 된다.
