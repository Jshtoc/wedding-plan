...
└── middleware.ts                    Edge middleware — 전역 인증 체크
```
...
- **미들웨어**: 반드시 `src/middleware.ts` (Next.js 규칙)
...
- 전역 미들웨어(`src/middleware.ts`)가 모든 경로에서 세션 쿠키를 검증한다.
...
`src/middleware.ts`에서 import하는 모든 파일(transitive 포함)은 Edge runtime에서 동작해야 한다.
...
만약 `middleware.ts`와 API 라우트가 공통 로직을 공유하고, 그 로직이 Node 모듈을 필요로 한다면:
...
6. `middleware.ts`에서 reachable한 코드는 Edge runtime 호환
