...
### Auth 팔레트 (로그인 전용, 라이트 미니멀)

```css
--color-sapphire: #4f46e5;  /* 인디고, 기본 액션 (CTA 버튼) */
--color-jade:     #0d9488;  /* 틸, 라벨/악센트 */
--color-pearl:    #e5e7eb;  /* 연한 그레이, 인풋 테두리/구분선 */
```

**Tailwind 유틸리티 예**:
- `bg-sapphire`, `text-sapphire`, `bg-sapphire/10`, `focus:ring-sapphire/10`
- `text-jade`, `text-jade/80`
- `border-pearl`, `border-pearl/70`
- 불투명도: `bg-sapphire/10` (아이콘 배경), `text-sapphire/80`
...
| 2. 버튼 CTA | `shadow-[0_4px_16px_-4px_rgba(79,70,229,0.35)]` | sapphire 제출 버튼 |

**팁**: 브랜드 색(sapphire, gold)을 낮은 알파로 섞은 shadow를 쓰면 CTA가 떠 있는 느낌을 줍니다. 너무 남용하면 촌스러워지니 메인 액션 버튼에만.
...
**Auth 버튼 (sapphire)**
```tsx
<button className="w-full h-14 bg-sapphire text-white rounded-2xl text-base font-semibold tracking-tight transition-all hover:bg-sapphire/90 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-[0_4px_16px_-4px_rgba(79,70,229,0.35)]">
  로그인
</button>
```
...
**Auth (light)**
```tsx
<form className="w-full max-w-[420px] bg-white border border-pearl/70 rounded-3xl p-8 sm:p-10 shadow-[0_2px_24px_rgba(15,23,42,0.04)]">
  ...
</form>
```
...
**Auth (light)**
```tsx
<div className="flex items-start gap-2 text-[13px] text-red-600 bg-red-50 border border-red-200 px-4 py-3 rounded-xl">
  <TwEmoji emoji="⚠️" size={14} className="flex-shrink-0 mt-0.5" />
  <span className="leading-relaxed">{error}</span>
</div>
```
...
**Auth (sapphire tint, 라이트 테마)**
```tsx
<div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-sapphire/10">
...
```tsx
// ❌ 임의 hex
<div className="bg-[#4f46e5] text-[#e5e7eb]">...</div>

// ✅ 토큰 사용
<div className="bg-sapphire text-white border-pearl">...</div>
```

```tsx
// ❌ 한 페이지에 두 팔레트 혼용
<div className="bg-bg">
  <button className="bg-sapphire">로그인</button>  {/* 왜 sapphire?? */}
</div>
...
└─ Auth (/login)
    ...
    ├─ 카드 배경 → bg-white
    ├─ 라벨 / 악센트 → text-jade
    ├─ CTA 버튼 → bg-sapphire text-white
    ├─ 인풋 테두리 → border-pearl, focus:border-sapphire focus:ring-sapphire/10
...
└─ 발광해야 한다 (sapphire CTA 등) → brand-color glow shadow
```
